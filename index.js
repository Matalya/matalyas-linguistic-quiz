...
function shuffle_array(old_array) {
    ...
    while (old_array.length > 0) {
        let randomindex = randint(old_array.length - 1);
...
function check_answer(user_input, question) {
    if (user_input.startsWith("debugserve ")) {
        let chosen_id = Number(user_input.substring(11));
        if (chosen_id > Q_COUNT || chosen_id < 1 || isNaN(chosen_id)) {
            print("No question with such index!")
        } else {
        question = questions.find(
            (question) => question.id == chosen_id
        );
        ...
        switch(question) {
            case "approximation":
                print("Sorry, haven't made it yet. Have the points anyway.")
